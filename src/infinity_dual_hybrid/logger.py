...
We intentionally avoid importing torch/tensorboard at module import time.
Torch import can be expensive and will slow (or appear to hang) `pytest --collect-only`.
Tensorboard support is resolved lazily only when enabled.
"""
...
class LoggerConfig:
    ...
    log_dir: str = "logs"
    run_dir: Optional[str] = None  # backwards-compat alias for log_dir
    ...
    def __post_init__(self) -> None:
        # Prefer run_dir if provided (backwards-compat)
        if self.run_dir is not None:
            self.log_dir = self.run_dir

        # Backwards/forwards compatibility with older config naming.
        # Allow cfg.csv / cfg.jsonl / cfg.tensorboard / cfg.console in addition to use_* names.
        for short, long in [
            ("csv", "use_csv"),
            ("jsonl", "use_jsonl"),
            ...
            if hasattr(self, short) and hasattr(self, long):
                # If long is default False and short is True, propagate.
                if getattr(self, long) is False and getattr(self, short) is True:
...
class UnifiedLogger:
    ...
    def __init__(self, cfg: LoggerConfig):
        ...
        if cfg.experiment_name is None:
            # If caller provided an explicit run_dir, log directly into it (no timestamp subfolder).
        ...
        if cfg.use_tensorboard:
            # Lazy import to avoid torch import cost when tensorboard is unused.
            ...
            try:
                ...
                SummaryWriter = _SummaryWriter  # type: ignore
