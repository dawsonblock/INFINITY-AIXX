...
NOTE ON IMPORTS
--------------
Some environments (CI, minimal installs, etc.) can take a *long* time importing
Mamba or may not have it installed at all. Importing it at module import time
can stall `pytest --collect-only` and other tooling.

We therefore lazy-import Mamba2 only when it is actually needed.
"""

# Optional Mamba2 import - vendored (vendor/mamba_ssm) or system-installed.
# Lazily resolved via _try_import_mamba2().
HAS_MAMBA = False
Mamba2 = None


def _try_import_mamba2() -> bool:
    """Best-effort import of Mamba2.

    Policy:
    - If CUDA is unavailable, prefer the lightweight local fallback to avoid
      slow Triton/CUDA import/compile paths during CPU runs and CI.
    - If CUDA is available, try mamba_ssm first, then fallback.

    Returns True if some Mamba2 implementation is available.
    """
    global HAS_MAMBA, Mamba2
    if Mamba2 is not None:
        return True
    if HAS_MAMBA:
        return True

    # CPU-first: avoid importing heavy CUDA/Triton paths when not needed.
    if not torch.cuda.is_available():
        from .mamba2_fallback import Mamba2 as _Mamba2  # type: ignore
        Mamba2 = _Mamba2
        HAS_MAMBA = True
        return True
...
def detect_ssm_backend(cfg: BackboneConfig) -> str:
    ...
    if use_mamba and _try_import_mamba2():
        return "mamba2"
    return "fallback"
...
class Mamba2Block(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # Ensure Mamba2 is available only when we actually instantiate a block.
        if not _try_import_mamba2():
            raise RuntimeError(
                "mamba-ssm (Mamba2) is not available. Set backbone.use_mamba: false, "
                "or install mamba-ssm / include vendor/mamba_ssm."
            )
...
class HybridSSMAttentionBackbone(nn.Module):
    ...
    def __init__(self, cfg: BackboneConfig):
        ...
        # Resolve Mamba2 availability (vendored or installed) before enforcing require_mamba2.
        if bool(getattr(cfg, "use_mamba", False)):
            _try_import_mamba2()
    ...
    def reset_state_env(self, env_ids: list[int]) -> None:
        """Best-effort per-env reset for stateful layers.

        Most runs here use full-forward mode (no persistent per-env inference cache).
        If a Mamba2 implementation (or a future change) introduces per-batch cached
        state, partial resets in vectorized envs must clear those indices to avoid
        state leakage across episodes.

        If no cached state exists, this is a no-op.
        """
        if not env_ids:
            return
        max_eid = max(env_ids)
        for layer in self.mamba_layers:
            st = getattr(layer, "_inference_state", None)
            if st is None:
                continue
            try:
                if isinstance(st, torch.Tensor) and st.shape[0] > max_eid:
                    for eid in env_ids:
                        st[eid].zero_()
                elif isinstance(st, dict):
                    for v in st.values():
                        if isinstance(v, torch.Tensor) and v.shape[0] > max_eid:
                            for eid in env_ids:
                                v[eid].zero_()
